...
function registerHandlebarsHelpers(): void {
  ...
  Handlebars.registerHelper('hasItems', (arr: unknown[]) => arr && arr.length > 0);
  ...
  Handlebars.registerHelper('pascalCase', (str: string) => {
    const camel = str.replace(/-([a-z])/g, (_, letter: string) => letter.toUpperCase());
    return camel.charAt(0).toUpperCase() + camel.slice(1);
  });

  // Helper para gerar comentário de baixa confiança
  Handlebars.registerHelper('lowConfidenceComment', (confidence: string) => {
  ...
  Handlebars.registerHelper('eachWithIndex', function(this: unknown, context: Record<string, unknown>[], options: Handlebars.HelperOptions) {
    let result = '';
...
export async function writeFileIdempotent(
  ...
  if (exists && effectiveCategory === 'merge' && filePath.endsWith('.json')) {
    ...
    const newJson = JSON.parse(content) as Record<string, unknown>;
 ...
 */
function prepareTemplateData(config: ProjectConfig, detected: DetectedConventions): Record<string, unknown> {
  return {
 ...
 */
export async function copyStaticSkills(cwd: string): Promise<void> {
  const staticSkills = [
 ...
 */
export async function copyValidateScript(cwd: string): Promise<void> {
 ...
 */
export async function copyAgent(cwd: string, config: ProjectConfig): Promise<void> {
  const templateData = {
    domains: config.domains,
    projectName: config.name,
    paths: config.paths,
 ...
 */
export async function regenerateCopilotInstructions(cwd: string = process.cwd()): Promise<void> {
  ...
  const conventions = YAML.parse(conventionsContent) as Record<string, unknown>;
 ...
 */
export async function scaffoldProject(config: ProjectConfig, detected: DetectedConventions): Promise<void> {
