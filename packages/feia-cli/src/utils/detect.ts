...
/**
 * Interface do package.json para tipagem
 */
...
/**
 * Interface do tsconfig.json para tipagem
 */
interface TsConfig {
  compilerOptions?: {
 ...
 */
function createResult<T>(value: T, confidence: Confidence): DetectionResult<T> {
 ...
 */
async function fileExists(cwd: string, ...paths: string[]): Promise<boolean> {
 ...
 */
async function detectFramework(_cwd: string, pkg: PackageJson | null): Promise<DetectionResult<Framework>> {
 ...
 */
async function detectCss(cwd: string, pkg: PackageJson | null): Promise<DetectionResult<CssLibrary>> {
 ...
 */
async function detectTestRunner(cwd: string, pkg: PackageJson | null): Promise<DetectionResult<TestRunner>> {
 ...
 * 4.4 Detecta linter
 */
async function detectLinter(cwd: string, pkg: PackageJson | null): Promise<DetectionResult<Linter>> {
 ...
 */
async function detectFormatter(cwd: string, pkg: PackageJson | null): Promise<DetectionResult<Formatter>> {
 ...
 */
function detectForms(pkg: PackageJson | null): DetectionResult<FormsLibrary> {
 ...
 */
function detectValidation(pkg: PackageJson | null): DetectionResult<ValidationLibrary> {
 ...
 */
function detectStateManagement(pkg: PackageJson | null): DetectionResult<StateManagement> {
 ...
 */
async function detectPathAlias(cwd: string): Promise<string | null> {
  const tsconfigPath = path.join(cwd, 'tsconfig.json');

  try {
    const tsconfig = await fs.readJson(tsconfigPath) as TsConfig;
 ...
 */
async function detectComponentPatterns(cwd: string): Promise<{
  ...
  let componentDeclaration: DetectionResult<ComponentDeclaration>;
  ...
  let exportStyle: DetectionResult<ExportStyle>;
 ...
 */
async function detectNamingConventions(cwd: string): Promise<{
  ...
  let namingFiles: DetectionResult<NamingConvention>;
  ...
  const namingComponents = createResult<NamingConvention>('PascalCase', 'high');
 ...
 */
async function detectPaths(cwd: string): Promise<DetectedPaths> {
  const paths: DetectedPaths = {
    components: null,
    pages: null,
    hooks: null,
    utils: null,
  ...
  const pathOptions: Record<keyof DetectedPaths, string[]> = {
  ...
  for (const [key, options] of Object.entries(pathOptions)) {
    for (const option of options) {
      if (await fs.pathExists(path.join(cwd, option))) {
        paths[key as keyof DetectedPaths] = option;
        break;
      }
    }
  }
 ...
 */
async function detectBarrelFiles(cwd: string): Promise<boolean> {
 ...
 */
export async function detectConventions(cwd: string = process.cwd()): Promise<DetectedConventions> {
