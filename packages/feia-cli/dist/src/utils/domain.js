 ...
 */
export async function updateOrvalConfig(name, specPath, baseUrl, cwd = process.cwd()) {
 ...
 */
export async function updateProjectYaml(name, specPath, baseUrl, cwd = process.cwd()) {
 ...
 */
export async function updateCustomInstance(name, baseUrl, cwd = process.cwd()) {
 ...
 */
export async function removeFromOrvalConfig(name, cwd = process.cwd()) {
    const orvalConfigPath = path.join(cwd, 'orval.config.ts');
    let content = await fs.readFile(orvalConfigPath, 'utf-8');
 ...
 */
export async function removeFromProjectYaml(name, cwd = process.cwd()) {
    const projectYamlPath = path.join(cwd, 'frontendspec', 'project.yaml');
    const content = await fs.readFile(projectYamlPath, 'utf-8');
    const project = YAML.parse(content);
 ...
 */
export async function removeFromCustomInstance(name, cwd = process.cwd()) {
    const customInstancePath = path.join(cwd, 'src', 'api', 'custom-instance.ts');
    let content = await fs.readFile(customInstancePath, 'utf-8');
    // Remove a função de instância
    const instanceName = `${name}Instance`;
    const pattern = new RegExp(`\\/\\*\\*[^*]*\\*\\/\\s*export const ${instanceName}[^;]*;\\s*\\};?`, 's');
    content = content.replace(pattern, '');
    // Também tenta remover formato mais simples
    const simplePattern = new RegExp(`export const ${instanceName}[^}]+\\};?\\s*`, 's');
